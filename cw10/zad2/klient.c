...
void rozlacz() {
    ...
    wyslij_wiadomosc(deskryptor_gniazda_serwera, LOGOUT, NULL, nazwa);
...
void* obsluga_odczytu(){
    read_input();
    wyslij_wiadomosc(deskryptor_gniazda_serwera, GAME_MOVE, input, nazwa);
    return NULL;
}
            ...
            while(input[0] == '.'){
                if( temp_temp_wiadomosc == NULL ){
                    ...
                    if( temp_temp_wiadomosc->typ == PING ){
                        wyslij_wiadomosc(deskryptor_gniazda_serwera, PING, NULL, nazwa);
                    }
                }
            }
...
void* obsluga_pingowania(){
    ...
    while(1){
        ...
        if(temp_wiadomosc == NULL){
            ...
            tick_serwera = 500;
            wyslij_wiadomosc(deskryptor_gniazda_serwera, PING, NULL, nazwa);   // odpinguj serwerowi
            //sleep(CZAS_SPANIA);
            usleep(10000 * CZAS_SPANIA);
        }else{
...
void polacz_z_serwerem(int flaga) {
    if(flaga == 1) {
        ...
        deskryptor_gniazda_serwera = socket(AF_UNIX, SOCK_DGRAM, 0);
        if(deskryptor_gniazda_serwera < 0) wyjscie_z_bledem("Nieudane otworzenie gniazda (Unix).");

        struct sockaddr_un gniazdo_klienta;
        gniazdo_klienta.sun_family = AF_UNIX;
        sprintf(gniazdo_klienta.sun_path, "%s", nazwa);
        bind(deskryptor_gniazda_serwera, (struct sockaddr*)&gniazdo_klienta, sizeof(gniazdo_klienta));
        ...
        struct sockaddr_in* adres_gniazda_inet = (struct sockaddr_in*)malloc(sizeof(struct sockaddr_in));
        ...
        deskryptor_gniazda_serwera = socket(AF_INET, SOCK_DGRAM, 0);
        if(deskryptor_gniazda_serwera < 0) wyjscie_z_bledem("Nieudane otworzenie gniazda (Inet).");

        struct sockaddr_in gniazdo_klienta;
        gniazdo_klienta.sin_family = AF_INET;
        gniazdo_klienta.sin_port = 0;
        gniazdo_klienta.sin_addr.s_addr = inet_addr(adres_gniazda_inet);
        bind(deskryptor_gniazda_serwera, (struct sockaddr*)&gniazdo_klienta, sizeof(gniazdo_klienta));

        if( connect(deskryptor_gniazda_serwera, (struct sockaddr*) adres_gniazda_inet, sizeof(struct sockaddr_in)) < 0 ) wyjscie_z_bledem("Nieudane polaczenie z serwerem (Inet).");
    }

    printf("\n");
    wypisz_wysrodkowane("_____ Proba zalogowania na serwer _____");

    wyslij_wiadomosc(deskryptor_gniazda_serwera, LOGIN_REQUEST, NULL, nazwa);

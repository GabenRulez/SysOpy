...
void inicjalizuj_serwer(){
    ...
    deskryptor_gniazda_unix = socket(AF_UNIX, SOCK_DGRAM, 0);
    ...
    deskryptor_gniazda_inet = socket(AF_INET, SOCK_DGRAM, 0);
...
void usun_klienta(char* name){
    for (int i=0; i<MAX_KLIENTOW; i++){
        if( klienci[i] != NULL && strcmp(klienci[i]->nazwa, name) == 0 ){
            free(klienci[i]);
            klienci[i] = NULL;
...
void* obsluga_sieci(){
    struct pollfd deskryptory[2];

    deskryptory[0].fd = deskryptor_gniazda_unix;
    deskryptory[0].events = POLLIN;

    deskryptory[1].fd = deskryptor_gniazda_inet;
    deskryptory[1].events = POLLIN;
    ...
    while(1){
        ...
        for(int i=0; i<2; i++){
            ...
            if( deskryptory[i].revents == POLLIN ){ // możliwy odczyt
                ...
                wiadomosc* msg = read_message_from(deskryptory[i].fd, addr, &len);

                if( msg->typ == LOGIN_REQUEST){      // login
                    ...
                    if( temp_indeks == -1 ){                // gdy nie ma wolnych miejsc
                        ...
                        send_message_to(deskryptor_gniazda_klienta, LOGIN_REJECTED, "Brak wolnych miejsc na serwerze. Sprobuj pozniej.");
                        continue;
                    }

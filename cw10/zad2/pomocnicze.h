...
typedef struct wiadomosc{
    char tresc[MAX_DLUGOSC_TEKSTU/2];
    TYP_WIADOMOSCI typ;
    char user[MAX_DLUGOSC_TEKSTU/2];
} wiadomosc;
...
void wyslij_wiadomosc(int deskryptor_gniazda, TYP_WIADOMOSCI typ, char* tresc, char* user){
    char* temp_string = (char*)calloc(MAX_DLUGOSC_TEKSTU, sizeof(char));
    sprintf(temp_string, "%d:%s:%s", (int) typ, tresc, user);
...
char* get_message_user(char* message){
    int tmp;
    char* content = (char*)calloc(MAX_DLUGOSC_TEKSTU-1, sizeof(char));
    char* user = (char*)calloc(MAX_DLUGOSC_TEKSTU-1, sizeof(char));
    sscanf(message, "%d:%[^:]:%[^:]", &tmp, content, user);
    return user;
}

wiadomosc* read_message_from(int sock_fd, struct sockaddr* addr, socklen_t* addrlen) {

    wiadomosc *temp = (wiadomosc *) malloc(sizeof(wiadomosc));
    char *temp_string = (char *) calloc(MAX_DLUGOSC_TEKSTU, sizeof(char));
    if (recvfrom(sock_fd, (void *) temp_string, MAX_DLUGOSC_TEKSTU, 0, addr, addrlen) < 0) wyjscie_z_bledem("recvfrom");
    sscanf(temp_string, "%d:%[^:]:%[^:]", (int *) &temp->typ, (char *) &temp->tresc, (char *) &temp->user);
    return temp;
}

void send_message_to(int deskryptor_gniazda, TYP_WIADOMOSCI typ, char* tresc, struct sockaddr* addr){
    char* temp_string = (char*)calloc(MAX_DLUGOSC_TEKSTU, sizeof(char));
    sprintf(temp_string, "%d:%s", (int) typ, tresc);
    if( sendto(deskryptor_gniazda, (void*) temp_string, MAX_DLUGOSC_TEKSTU, 0, addr, sizeof(struct sockaddr)) < 0 ) wyjscie_z_bledem("Nieudany zapis do gniazda.");
    free(temp_string);
}
